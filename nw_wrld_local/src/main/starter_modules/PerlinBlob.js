...
class PerlinBlob extends ModuleBase {
  ...
  init() {
    ...
    const sketch = (p) => {
      ...
      p.draw = () => {
        ...
        const maxRadius = (Math.min(p.width, p.height) * 0.8 * this.p_scaleObj) / 2;
        const minStrokeWeight = 0.1 * this.p_strokeWeight;
        const maxStrokeWeight = 2 * this.p_strokeWeight;
        ...
        for (let i = 0; i < numLayers; i++) {
          ...
          if (applyCompression) {
            ...
            minCompression = 1 - this.p_compression;
          }
          ...
          this.noiseOffsets[i] += this.p_noiseSpeed;
        }
      };
    };
  ...
  // --- MAPPED DATA METHODS ---
  // Received from SC via /bio/nutrient
  updateNutrient({ nutrientValue, isRemote = false }) {
    // nutrientValue is mapped 0.0-1.0 from SC
  ...
  setVolume({ velocity }) { this.p_scaleObj = 0.5 + (velocity * 1.5); } // 0.5 to 2.0x scale
  setPitchShift({ velocity }) { this.p_noiseSpeed = 0.001 + (velocity * 0.02); } // Faster morphing
  setTimeDilation({ velocity }) { } // Handled globally by nw_wrld usually, or ignore here
  setSpectralShift({ velocity }) { this.p_colorHueOff = velocity * 360; } // Shift through rainbow
  setSpatialSpread({ velocity }) { this.p_strokeWeight = 0.1 + (velocity * 3.0); } // Thicker lines
  setTextureDepth({ velocity }) { this.p_compression = velocity; } // More "squished" blob layers 
