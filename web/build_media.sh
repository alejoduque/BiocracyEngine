...
# ── Archivo ─────────────────────────────────────────────────────────────────
# Las 22 capturas de la cámara trampa y 36 grabaciones AudioMoth que el
# escenario sigue mostrando después de los cuatro de arriba (ver
# build_archivo.py). Lee ../corpus, que tiene que ser el corpus real: en main,
# corpus/audible son marcadores vacíos, y ahí hay que decirle dónde está con
# BIOCRACY_CORPUS.
command -v python3 >/dev/null || { echo "❌ falta python3"; exit 1; }
if [ $FORCE -eq 1 ]; then python3 "$HERE/build_archivo.py" --force
else python3 "$HERE/build_archivo.py"; fi
