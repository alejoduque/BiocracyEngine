...
# ${TCUT[@]+…}: con set -u, el bash 3.2 de macOS da por no definido un arreglo
# vacío, y TCUT lo está cuando no se pasa duración (ver deploy.sh).
meas=$(ffmpeg -hide_banner -ss "$SS" ${TCUT[@]+"${TCUT[@]}"} -i "$SRC" -ac 2 \
...
ffmpeg -v error -ss "$SS" ${TCUT[@]+"${TCUT[@]}"} -i "$SRC" -ac 2 \
